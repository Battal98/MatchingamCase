...
public static class ExtentionsHelper
{
    ...
    public static K ToPoolType<T,K>(this T changeType)
    {
        if (Enum.IsDefined(typeof(K), changeType.ToString()))
        {
            return (K)Enum.Parse(typeof(K), changeType.ToString());
        }
        else
        {
            // Dönüþüm yapýlacak deðer PoolType'da tanýmlanmamýþsa varsayýlan deðeri döndürür.
            return default; // veya baþka bir varsayýlan deðer
        }
    }
